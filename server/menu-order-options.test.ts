...
describe('kids, observacao e meio prato nos dois sistemas', () => {
  it('usa 60% para pratos elegiveis, Festival e itens com permissao explicita', () => {
    ...
    expect(modulo).toContain("return Boolean(produto?.servePara2) || produto?.permiteMeioPrato === true || String(produto?.categoria || '').trim().toLowerCase() === 'festival'");
    ...
    expect(modulo).toContain('personalizado.servePara2 = Boolean(produto?.servePara2)');

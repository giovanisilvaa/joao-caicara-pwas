/* Base de controle de acesso do Garçom. Modo compatibilidade: não bloqueia o fluxo atual. */
(() => {
  ...
  function definirPerfil(perfil, origem = 'aplicacao') {
    ...
    window.dispatchEvent(new CustomEvent('garcom:perfil-alterado', { detail: { perfil, origem } }));
  }
  ...
  async function consultarPerfilRemoto(uid) {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('garcom:perfil-remoto-consultado', {
        detail: {
          uid,
      ...
      if (estado.sessao.uid !== uid) return null;
      console.warn('Não foi possível consultar o perfil remoto do Garçom; acesso atual foi preservado:', erro);
      estado.perfilRemoto.carregado = true;
      estado.perfilRemoto.encontrado = null;
  ...
  function identificarSessaoFirebase(user) {
    ...
    window.dispatchEvent(new CustomEvent('garcom:sessao-identificada', {
      detail: {
  ...
  function conectarSessaoFirebase() {
    try {
      ...
      auth.onAuthStateChanged(
        ...
        erro => {
          console.warn('Não foi possível identificar a sessão Firebase do Garçom:', erro);
          estado.sessao.authReady = true;
      ...
      console.warn('Falha não bloqueante ao conectar controle de acesso do Garçom ao Firebase:', erro);
      estado.sessao.origem = 'firebase-erro';
      return false;
    }
  }
  ...
  function ativarControleEstrito() {
    ...
    window.dispatchEvent(new CustomEvent('garcom:controle-acesso-ativado', { detail: { perfil: estado.perfilAtual } }));
  }

  window.GarcomAcesso = Object.freeze({
    PERFIS,
    estado,
    pode,
    definirPerfil,
